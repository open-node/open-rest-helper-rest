...
var findAllOpts = function(Model, params, isAll) {
  ...
  if (searchOrs.length) where.$or = [[utils.mergeSearchOrs(searchOrs), ['']]];
...
// findOptFilter 的处理
var findOptFilter = function(params, name, where, col = name) {
  var value;
  ...
  _.each(['gt', 'gte', 'lt', 'lte'], function(x) {
    if (!_.isString(params[name + '_' + x])) return;
...
var searchOpt = function(Model, searchStr, qstr, as = '') {
  ...
  var q = _.split(qstr.trim(), ' ', 5);
  var searchs = _.split(searchStr, ',');
  var $ors = [];
  if (!q.length) return;
  if (!q[0]) return;
